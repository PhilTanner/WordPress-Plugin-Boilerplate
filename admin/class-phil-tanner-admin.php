...
class Phil_Tanner_Admin {
   ...
   * @since    1.0.0
	...
	public static function get_git_branch() {
		if( !file_exists( PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'HEAD' ) ) {
			return false;
		} else {
			return trim(substr(file_get_contents(PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'HEAD'), 16));
		}
	}
	...
	public static function get_git_repo_url() {
		if( !file_exists( PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'config' ) ) {
			return false;
		} else {
			preg_match_all('/url = (.*)/',file_get_contents(PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'config'),$m);
			if( count($m) < 2 ) {
				return false;
			}
	...
	public static function get_git_commit_date() {
		...
		if( !file_exists( PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'refs'.DIRECTORY_SEPARATOR.'heads'.DIRECTORY_SEPARATOR.$branch ) ) {
			return false;
		} else {
			return date('c', filemtime(PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'refs'.DIRECTORY_SEPARATOR.'heads'.DIRECTORY_SEPARATOR.$branch));
		}
	}
	...
	public static function get_git_commit_hash() {
		...
		if( !file_exists( PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'refs'.DIRECTORY_SEPARATOR.'heads'.DIRECTORY_SEPARATOR.$branch ) ) {
			...
			return file_get_contents(PLUGIN_NAME_PATH.DIRECTORY_SEPARATOR.'.git'.DIRECTORY_SEPARATOR.'refs'.DIRECTORY_SEPARATOR.'heads'.DIRECTORY_SEPARATOR.$branch);
		}
	}
